...
function BookingModal({
  ...
  // ═══ VERIFY PAYMENT (Razorpay/Cashfree) ═══
  ...
  // ═══ UPI QR: "I've Paid" Handler ═══
  const handleUPIConfirm = async () => {
    if (!upiBookingInfo || !upiQR) return;
    setPaymentVerified(true);
    try {
      // Mark payment as paid (manual verification)
      ...
      await triggerNotifications(
        ...
        upiBookingInfo.roomNumber
      );

      setUpiQR(null);
      setConfirmation({ ref: upiBookingInfo.bookingRef, name: name.trim(), paid: true });
    } catch (err: any) {
      console.error(err);
      setError("Could not confirm payment. Please contact the hotel.");
      ...
      <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md flex items-center justify-center z-[100] p-4">
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
          ...
          <div className="p-8 space-y-6">
            ...
            <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl">
              <p className="text-[11px] text-blue-800 leading-relaxed">
                <strong>ℹ️ How it works:</strong> Pay using the QR code or UPI
                app. After payment is done, click <strong>"I've Paid"</strong>{" "}
                below. The hotel will verify and confirm your booking shortly.
              </p>
            </div>
          </div>
          ...
          <div className="px-8 py-5 border-t border-slate-100 bg-slate-50 flex items-center justify-between gap-3">
            ...
            >
              {paymentVerified ? "Verifying..." : "✓ I've Paid"}
            </button>
          </div>
        </div>
      </div>
