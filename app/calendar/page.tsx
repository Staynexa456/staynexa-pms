...
export default function CalendarPage() {
  ...
  const generateBillHtml = (b: any, type: "normal" | "company", companyName?: string, companyGst?: string, companyEmail?: string, companyPhone?: string, companyAddress?: string) => {
    ...
    return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Tax Invoice</title><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'Helvetica Neue',Arial,sans-serif;padding:25px;color:#1e293b;background:#fff;}.invoice{max-width:820px;margin:0 auto;border:1px solid #cbd5e1;border-radius:12px;padding:36px;box-shadow:0 4px 24px rgba(0,0,0,0.04);}.header{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid ${accentColor};padding-bottom:22px;margin-bottom:26px;}.brand{display:flex;gap:16px;align-items:flex-start;}.brand img{height:60px;object-fit:contain;}.brand h1{color:${accentColor};font-size:26px;font-weight:800;margin-bottom:4px;}.brand p{font-size:12px;color:#64748b;line-height:1.5;}.brand .gst{font-weight:700;color:#0f172a;font-size:12px;margin-top:4px;}.inv-meta{text-align:right;}.inv-meta .title{font-size:22px;font-weight:800;color:${accentColor};text-transform:uppercase;letter-spacing:2px;margin-bottom:8px;}.inv-meta .meta-row{font-size:12px;color:#475569;margin-bottom:3px;}.inv-meta .meta-row strong{color:#0f172a;}.billto-grid{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:26px;}.info-box{background:${isCompany ? "#eff6ff" : "#f8fafc"};border:1px solid ${isCompany ? "#bfdbfe" : "#e2e8f0"};border-radius:10px;padding:16px;}.info-box h3{font-size:10px;text-transform:uppercase;color:${accentColor};margin-bottom:8px;font-weight:800;letter-spacing:1.5px;}.info-box .name{font-size:16px;font-weight:800;color:#0f172a;margin-bottom:4px;}.info-box .line{font-size:12px;color:#475569;line-height:1.6;}.info-box .line strong{color:#0f172a;}table.items{width:100%;border-collapse:collapse;margin-bottom:20px;}table.items th{padding:12px 16px;text-align:left;font-size:11px;background:${accentColor};color:#fff;font-weight:700;text-transform:uppercase;letter-spacing:1px;}table.items td{padding:12px 16px;font-size:13px;border-bottom:1px solid #e2e8f0;color:#334155;}table.items th:last-child,table.items td:last-child{text-align:right;}.totals-wrap{display:flex;justify-content:flex-end;margin-bottom:26px;}.totals{width:340px;border-collapse:collapse;}.totals td{padding:10px 16px;font-size:13px;border-bottom:1px solid #f1f5f9;text-align:right;color:#475569;}.totals td:first-child{text-align:left;}.totals tr.grand td{font-weight:800;background:${isCompany ? "#eff6ff" : "#f0fdfa"};color:${accentColor};font-size:15px;border-bottom:none;}.totals tr.paid td{font-weight:600;color:#059669;}.totals tr.balance td{font-weight:800;background:#fef2f2;color:#b91c1c;font-size:14px;}.terms{margin-top:30px;border-top:1px solid #e2e8f0;padding-top:16px;}.terms h4{font-size:10px;text-transform:uppercase;letter-spacing:1.5px;color:#94a3b8;font-weight:800;margin-bottom:8px;}.terms p{font-size:11px;color:#64748b;line-height:1.7;white-space:pre-wrap;}.footer{text-align:center;margin-top:30px;padding-top:16px;border-top:1px solid #f1f5f9;}.footer p{font-size:11px;color:#94a3b8;}</style></head><body><div class="invoice"><div class="header"><div class="brand">${hotelLogo ? `<img src="${hotelLogo}" alt="Logo" />` : ""}<div><h1>${hotelName}</h1>${hotelAddress ? `<p>${hotelAddress}</p>` : ""}${hotelPhone ? `<p>📞 ${hotelPhone}</p>` : ""}${hotelGst ? `<p class="gst">GSTIN: ${hotelGst}</p>` : ""}</div></div><div class="inv-meta"><div class="title">Tax Invoice</div><div class="meta-row"><strong>No:</strong> ${invoiceNo}</div><div class="meta-row"><strong>Date:</strong> ${new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</div></div></div><div class="billto-grid">${isCompany ? `<div class="info-box"><h3>Bill To (Company)</h3><div class="name">${companyName || "—"}</div>${companyAddress ? `<div class="line">${companyAddress}</div>` : ""}${companyEmail ? `<div class="line">✉ ${companyEmail}</div>` : ""}${companyPhone ? `<div class="line">📞 ${companyPhone}</div>` : ""}${companyGst ? `<div class="line"><strong>GSTIN: ${companyGst}</strong></div>` : ""}</div><div class="info-box"><h3>Guest & Stay</h3><div class="name">${guest.name || "Guest"}</div><div class="line"><strong>Room:</strong> ${b.roomNumber || "—"} (${b.roomType || "—"})</div><div class="line"><strong>Check-in:</strong> ${b.checkIn || "—"}</div><div class="line"><strong>Check-out:</strong> ${b.checkOut || "—"}</div></div>` : `<div class="info-box"><h3>Bill To</h3><div class="name">${guest.name || "Guest"}</div>${guest.address ? `<div class="line">${guest.address}</div>` : ""}${guest.phone ? `<div class="line">📞 ${guest.phone}</div>` : ""}${guest.email ? `<div class="line">✉ ${guest.email}</div>` : ""}</div><div class="info-box"><h3>Stay Details</h3><div class="line"><strong>Room:</strong> ${b.roomNumber || "—"} (${b.roomType || "—"})</div><div class="line"><strong>Check-in:</strong> ${b.checkIn || "—"}</div><div class="line"><strong>Check-out:</strong> ${b.checkOut || "—"}</div></div>`}</div><table class="items"><thead><tr><th style="width:60%;">Description</th><th style="width:10%;text-align:center;">Qty</th><th style="width:30%;">Amount (Rs.)</th></tr></thead><tbody>${itemRows}</tbody></table><div class="totals-wrap"><table class="totals"><tr><td>Sub Total</td><td>₹${(amount + addonsSubtotal).toFixed(2)}</td></tr>${tax + addonsTax > 0 ? `<tr><td>CGST</td><td>₹${((tax + addonsTax) / 2).toFixed(2)}</td></tr><tr><td>SGST</td><td>₹${((tax + addonsTax) / 2).toFixed(2)}</td></tr>` : ""}<tr class="grand"><td>Grand Total</td><td>₹${totalAmount.toFixed(2)}</td></tr><tr class="paid"><td>Payment Made</td><td>₹${paid.toFixed(2)}</td></tr><tr class="balance"><td>Balance Due</td><td>₹${balance.toFixed(2)}</td></tr></table></div>${hotelTerms ? `<div class="terms"><h4>Terms & Conditions</h4><p>${hotelTerms}</p></div>` : ""}<div class="footer"><p>Thank you for staying with us! · Generated by Staynexa PMS</p></div></div></body></html>`;
  };
    ...
    <div className="flex flex-col bg-gradient-to-br from-slate-50 via-white to-slate-50 min-h-screen">
      ...
      {createOpen && (
        <CreateReservationModal
          ...
          onBlockRoom={async (data: { roomNumber: string; checkIn: string; checkOut: string; reason: string }) => {
  ...
  />
      ...
      {datePickerOpen && (
        <DateRangePicker
          ...
          onApply={(start: string, end: string) => { setStartDate(start); setDatePickerOpen(false); }}
      ...
      {cellContextMenu && (
        <>
          ...
          <div className="fixed z-[61] bg-white border border-slate-200 rounded-xl shadow-2xl py-1.5 min-w-[200px] overflow-hidden" style={{ left: `${Math.min(cellContextMenu.x, window.innerWidth - 220)}px`, top: `${Math.min(cellContextMenu.y, window.innerHeight - 140)}px` }}>
            <div className="px-3 py-1.5 border-b border-slate-100"><p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Room {cellContextMenu.roomNumber} · {prettyDate(fmt(cellContextMenu.date))}</p></div>
