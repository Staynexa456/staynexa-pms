...
export default function PublicBookingPage() {
  ...
  const checkAllAvailability = useCallback(async () => {
    ...
    } catch (err) { console.error(err); } finally { setCheckingAvail(false); }
  ...
  if (loading) return ( <div className="min-h-screen flex items-center justify-center bg-white"><div className="text-center"><div className="w-12 h-12 mx-auto mb-4 rounded-full border-2 border-slate-200 border-t-slate-900 animate-spin" /><p className="text-xs text-slate-400 font-medium tracking-widest uppercase">Loading</p></div></div> );
  if (notFound) return ( <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6"><div className="text-center max-w-md"><p className="text-6xl mb-4">🏨</p><h1 className="text-2xl font-bold text-slate-900 mb-2">Hotel Not Found</h1><p className="text-sm text-slate-500">The booking link doesn't match any hotel.</p></div></div> );
  if (disabled) return ( <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6"><div className="text-center max-w-md"><p className="text-6xl mb-4">⏸</p><h1 className="text-2xl font-bold text-slate-900 mb-2">Booking Temporarily Unavailable</h1><p className="text-sm text-slate-500">Please contact the hotel directly.</p></div></div> );
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="absolute top-0 left-0 right-0 z-40 px-6 lg:px-16 py-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="hidden md:block">
            ...
            {hotel?.city && <p className="text-[10px] text-white/60 uppercase tracking-[0.2em]">{hotel.city}{hotel.state ? `, ${hotel.state}` : ""}</p>}
          </div>
        </div>
      ...
      {config?.show_hero_banner !== false ? (
        <section className="relative h-[600px] overflow-hidden">
          <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: config?.hero_banner_url ? `url(${config.hero_banner_url})` : roomTypes[0]?.photo_url ? `url(${roomTypes[0].photo_url})` : "linear-gradient(135deg, #1e293b 0%, #0f172a 100%)" }} />
          <div className="absolute inset-0" style={{ background: `linear-gradient(to bottom, rgba(15,23,42,${config?.hero_overlay_opacity || 0.6}) 0%, rgba(15,23,42,${(config?.hero_overlay_opacity || 0.6) * 0.7}) 50%, rgba(15,23,42,${config?.hero_overlay_opacity || 0.6}) 100%)` }} />
          <div className="relative h-full flex flex-col items-center justify-center text-center px-6">
            <p className="text-[11px] tracking-[0.4em] uppercase text-white/70 mb-4 font-medium">Welcome to</p>
            <h1 className="text-4xl md:text-6xl font-serif font-semibold text-white mb-4 max-w-3xl leading-tight">{config?.hero_title || hotel?.name}</h1>
            <div className="w-16 h-[1px] bg-white/40 mx-auto mb-4" />
            <p className="text-base md:text-lg text-white/80 max-w-2xl font-light italic">{config?.hero_subtitle || "An unforgettable stay awaits you"}</p>
          </div>
        </section>
      ) : (
        <section className="relative pt-32 pb-20 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
          <div className="relative text-center px-6">
            <p className="text-[11px] tracking-[0.4em] uppercase text-white/60 mb-4 font-medium">Welcome to</p>
            <h1 className="text-4xl md:text-6xl font-serif font-semibold text-white mb-4 max-w-3xl mx-auto leading-tight">{config?.hero_title || hotel?.name}</h1>
            <div className="w-16 h-[1px] bg-white/30 mx-auto mb-4" />
            <p className="text-base md:text-lg text-white/70 max-w-2xl mx-auto font-light italic">{config?.hero_subtitle || "An unforgettable stay awaits you"}</p>
          </div>
        </section>
      )}
        ...
        <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.25)] border border-slate-100 p-6">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div className="flex items-end">
              <button onClick={() => { checkAllAvailability(); scrollToRooms(); }} disabled={checkingAvail} className="w-full py-3.5 rounded-xl text-[11px] font-bold text-white transition hover:opacity-90 disabled:opacity-50 uppercase tracking-[0.2em]" style={{ background: themeColor }}>
      ...
      <section id="rooms-section" className="px-6 lg:px-16 py-20 max-w-6xl mx-auto">
        ...
        <div className="space-y-8">
          {roomTypes.map((room) => {
            ...
            return (
              <div key={room.room_type} className={`group bg-white rounded-2xl border overflow-hidden transition-all duration-300 ${isAvailable ? "border-slate-200 hover:border-slate-300 hover:shadow-[0_20px_50px_-20px_rgba(0,0,0,0.15)]" : "border-slate-100 opacity-60"}`}>
                <div className="flex flex-col lg:flex-row">
                  <div className="lg:w-[380px] h-64 lg:h-auto bg-slate-100 shrink-0 relative overflow-hidden">
                    {room.photo_url ? <img src={room.photo_url} alt={room.room_type} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" /> : <div className="w-full h-full flex items-center justify-center text-5xl text-slate-300">🛏️</div>}
                    {!isAvailable && <div className="absolute inset-0 bg-slate-900/70 flex items-center justify-center"><span className="px-5 py-2.5 bg-white text-slate-900 rounded-full text-[10px] font-bold uppercase tracking-[0.2em]">Sold Out</span></div>}
                  </div>
                    ...
                    <div className="flex items-start justify-between gap-4 mb-3">
                      <div>
                        ...
                        <div className="flex items-center gap-4 mt-2 text-[11px] font-medium text-slate-500 uppercase tracking-wider">
                          <span className="flex items-center gap-1.5"><span className="w-1 h-1 rounded-full bg-slate-400" />Max {room.max_adults} Adults</span>
                          {room.max_children > 0 && <span className="flex items-center gap-1.5"><span className="w-1 h-1 rounded-full bg-slate-400" />Max {room.max_children} Children</span>}
                        </div>
                      </div>
                      {isAvailable && avail !== undefined && <span className="text-[10px] font-bold px-3 py-1.5 rounded-full bg-emerald-50 text-emerald-700 uppercase tracking-wider border border-emerald-100 shrink-0">{avail} Left</span>}
                    </div>
                    {room.description && <p className="text-sm text-slate-500 mb-6 line-clamp-2 leading-relaxed">{room.description}</p>}
                      ...
                      {ratePlans.length === 0 && (
                        <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
                          ...
                          <div className="text-right">
                            ...
                            <button disabled={!isAvailable} onClick={() => setBookingRoom({ room, plan: { code: "STD", name: "Standard Rate", rate_difference: 0, min_length_of_stay: 1 } })} className="px-6 py-2 rounded-lg text-[10px] font-bold text-white uppercase tracking-[0.15em] disabled:opacity-50 transition hover:opacity-90" style={{ background: themeColor }}>Book Now</button>
                          </div>
                        </div>
                      )}
                      {ratePlans.map((plan) => {
                          ...
                          <div key={plan.code} className="flex items-center justify-between p-4 bg-slate-50 hover:bg-slate-100 rounded-xl transition border border-transparent hover:border-slate-200">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                <span className="text-[9px] font-bold px-2 py-0.5 rounded text-white uppercase tracking-wider" style={{ background: themeColor }}>{plan.code}</span>
                            ...
                            <div className="text-right ml-4 shrink-0">
                              ...
                              <button disabled={!isAvailable} onClick={() => setBookingRoom({ room, plan })} className="px-6 py-2 rounded-lg text-[10px] font-bold text-white uppercase tracking-[0.15em] disabled:opacity-50 transition hover:opacity-90" style={{ background: themeColor }}>Book Now</button>
                            </div>
                          </div>
                        );
                      })}
          ...
          <div className="max-w-6xl mx-auto">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <h2 className="text-3xl md:text-4xl font-serif font-semibold text-slate-900 mb-5">{config?.about_title || "Welcome to Our Hotel"}</h2>
              ...
              {config?.about_image_url && <div className="relative h-[400px] rounded-2xl overflow-hidden shadow-xl"><img src={config.about_image_url} alt="About" className="w-full h-full object-cover" /></div>}
            </div>
          </div>
        </section>
      )}
        ...
        <section id="amenities-section" className="px-6 lg:px-16 py-20 bg-white">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-12">
              ...
              <h2 className="text-3xl md:text-4xl font-serif font-semibold text-slate-900">Hotel Amenities</h2>
              <div className="w-16 h-[1px] bg-slate-300 mx-auto mt-5" />
            </div>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {config.amenities.map((amenity, idx) => (
                <div key={idx} className="bg-slate-50 rounded-xl p-5 border border-slate-100 hover:border-slate-300 transition text-center">
        ...
        <section id="gallery-section" className="px-6 lg:px-16 py-20 bg-slate-50">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-12">
              ...
              <h2 className="text-3xl md:text-4xl font-serif font-semibold text-slate-900">{config?.gallery_title || "Photo Gallery"}</h2>
              <div className="w-16 h-[1px] bg-slate-300 mx-auto mt-5" />
            </div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {config.gallery_images.map((url, idx) => (
                <div key={idx} className={`rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-300 cursor-pointer ${idx === 0 ? "col-span-2 row-span-2 h-[400px]" : "h-[190px]"}`}>
        ...
        <section className="px-6 lg:px-16 py-20 bg-white">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-12">
              ...
              <h2 className="text-3xl md:text-4xl font-serif font-semibold text-slate-900">What Our Guests Say</h2>
              <div className="w-16 h-[1px] bg-slate-300 mx-auto mt-5" />
            </div>
              ...
              {config.testimonials.map((t: any, idx: number) => (
                <div key={idx} className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-12 h-12 rounded-full flex items-center justify-center text-white font-bold" style={{ background: themeColor }}>{(t.name || "G").charAt(0).toUpperCase()}</div>
                    <div>
                      <p className="text-sm font-bold text-slate-800">{t.name || "Guest"}</p>
                      <p className="text-[10px] text-amber-500">{"★".repeat(t.rating || 5)}</p>
                    </div>
                  </div>
        ...
        <section id="map-section" className="px-6 lg:px-16 py-20 bg-slate-50">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="rounded-2xl overflow-hidden shadow-lg border border-slate-200 h-[450px]">
              <iframe src={config.map_embed_url} className="w-full h-full border-0" allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" />
            </div>
          </div>
        </section>
      )}
        ...
        <section className="px-6 lg:px-16 py-20 bg-white">
          <div className="max-w-3xl mx-auto">
            <div className="text-center mb-12">
              ...
              <h2 className="text-3xl md:text-4xl font-serif font-semibold text-slate-900">Frequently Asked Questions</h2>
              <div className="w-16 h-[1px] bg-slate-300 mx-auto mt-5" />
            </div>
            <div className="space-y-3">
              {config.faqs.map((faq: any, idx: number) => (
                <div key={idx} className="bg-slate-50 rounded-xl border border-slate-100 overflow-hidden">
                  <button onClick={() => setActiveFaq(activeFaq === idx ? null : idx)} className="w-full text-left px-6 py-4 flex items-center justify-between gap-4 hover:bg-slate-100 transition">
                    <span className="text-sm font-semibold text-slate-800">{faq.question || ""}</span>
                    <span className="text-slate-400 text-lg shrink-0">{activeFaq === idx ? "−" : "+"}</span>
                  </button>
                  {activeFaq === idx && <div className="px-6 pb-4 text-sm text-slate-600 leading-relaxed">{faq.answer || ""}</div>}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-6 lg:px-16 py-16">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
            <div className="md:col-span-2">
              ...
              {config?.contact_address && <p className="text-sm text-slate-400 leading-relaxed mb-4">{config.contact_address}</p>}
              <div className="flex items-center gap-3 mt-4">
                {config?.facebook_url && <a href={config.facebook_url} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition text-sm">📘</a>}
                {config?.instagram_url && <a href={config.instagram_url} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition text-sm">📷</a>}
                {config?.whatsapp_number && <a href={`https://wa.me/${config.whatsapp_number.replace(/\D/g, "")}`} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition text-sm">💬</a>}
              </div>
            </div>
            <div>
              ...
              {config?.check_in_time && <p className="text-xs text-slate-500 mt-3">Check-in: {config.check_in_time}</p>}
          ...
          <div className="mt-12 pt-8 border-t border-white/10 text-center">
            ...
            <p className="text-[11px] text-slate-600 mt-2 tracking-wider">Powered by <span className="text-slate-400 font-medium">Staynexa PMS</span></p>
          </div>
        </div>
      </footer>
...
function BookingModal({
      ...
      if (data.gateway === "razorpay") {
        ...
        const options = {
          ...
          handler: async function (response: any) {
            await verifyPayment({ hotelId: hotel.id, gateway: "razorpay", orderId: response.razorpay_order_id, paymentId: response.razorpay_payment_id, signature: response.razorpay_signature, bookingId, bookingRef, roomNumber });
          },
  ...
  const handleUPIConfirm = async () => {
    ...
    setVerifying(true);
    try {
      const res = await fetch("/api/payments/verify", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ hotelId: hotel.id, gateway: "upi_qr", orderId: upiQR.orderId, bookingId: upiBookingInfo.bookingId, bookingRef: upiBookingInfo.bookingRef, roomNumber: upiBookingInfo.roomNumber }) });
      const data = await res.json();
      if (!data.success) throw new Error(data.error || "Verification failed");
      await triggerNotifications(upiBookingInfo.bookingId, upiBookingInfo.bookingRef, upiBookingInfo.roomNumber);
      setUpiQR(null);
      setConfirmation({ ref: upiBookingInfo.bookingRef, name: name.trim(), paymentStatus: "pending" });
    } catch (err: any) {
      console.error(err);
        ...
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
          <div className="px-8 py-6 border-b border-slate-100 bg-slate-50">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => { setUpiQR(null); setPaymentProcessing(false); setSubmitting(false); }} disabled={verifying} className="w-8 h-8 rounded-full bg-white hover:bg-slate-100 text-slate-400 hover:text-slate-700 flex items-center justify-center transition border border-slate-200 disabled:opacity-50">×</button>
            </div>
          </div>
          <div className="p-8 space-y-6">
            <div className="flex flex-col items-center">
              <div className="p-4 bg-white rounded-2xl border-2 border-slate-100 shadow-sm"><img src={upiQR.qrCodeUrl} alt="UPI QR Code" className="w-64 h-64" /></div>
            ...
            <a href={upiQR.deepLink} className="block w-full py-3.5 rounded-xl text-xs font-bold text-white text-center uppercase tracking-[0.2em] bg-gradient-to-r from-teal-500 to-emerald-500 hover:from-teal-600 hover:to-emerald-600 transition shadow-lg shadow-teal-500/30">📱 Open UPI App</a>
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl">
              <p className="text-[11px] text-amber-800 leading-relaxed"><strong>ℹ️ Important:</strong> After paying, click <strong>"I've Paid"</strong> below. The hotel will verify your payment and confirm the booking.</p>
            </div>
          </div>
      ...
      <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md flex items-center justify-center z-[100] p-4">
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
          <div className="p-10 text-center border-b border-slate-100">
            ...
            <p className="text-sm text-slate-500">{confirmation.paymentStatus === "paid" ? "Payment confirmed. A confirmation has been sent." : confirmation.paymentStatus === "pending" ? "Booking received. Awaiting payment verification from hotel." : "A confirmation has been sent to you."}</p>
          </div>
          <div className="p-8 space-y-5">
            ...
            <div className="p-5 bg-slate-50 rounded-2xl space-y-3 text-sm border border-slate-100">
              ...
              <div className="flex justify-between">
                ...
                {confirmation.paymentStatus === "paid" ? <span className="font-bold text-emerald-600">✅ Paid</span> : confirmation.paymentStatus === "pending" ? <span className="font-bold text-amber-600">⏳ Pending Verification</span> : <span className="font-bold text-slate-500">Pay at Hotel</span>}
              </div>
            </div>
          </div>
    ...
    <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md flex items-center justify-center z-[100] p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[92vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-6">
          ...
          {error && <div className="p-4 bg-rose-50 border border-rose-200 rounded-xl"><p className="text-xs text-rose-700 font-medium">⚠ {error}</p></div>}
        </div>
        <div className="px-8 py-5 border-t border-slate-100 bg-slate-50 flex items-center justify-between gap-3">
