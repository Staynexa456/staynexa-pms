...
// Get rate for a specific date + category + plan
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => shiftDates(-7)} className="px-3 py-2 border border-cream-dark rounded-lg text-sm font-medium text-navy hover:bg-cream transition">
            ← Prev
          </button>
          <button onClick={() => setStartDate("2026-09-12")} className="px-4 py-2 border border-cream-dark rounded-lg text-sm font-medium text-navy hover:bg-cream transition">
            This week
          </button>
          <button onClick={() => shiftDates(7)} className="px-3 py-2 border border-cream-dark rounded-lg text-sm font-medium text-navy hover:bg-cream transition">
            Next →
      ...
      <div className="bg-white border border-cream-dark rounded-xl p-4 mb-6 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-3">
          <FilterSelect label="Rates / Inventory / Restrictions" options={["Rates", "Inventory", "Restrictions"]} />
          <div>
          ...
          <FilterSelect label="Room Categories" options={["All", "Deluxe Room", "Superior King Room", "Executive Suite Room", "Family Room"]} />
          ...
          <table className="w-full min-w-[900px]">
            ...
            <tbody>
              ...
              <tr className="border-b border-cream-dark">
                <td className="px-4 py-3 text-sm font-medium text-navy">Occupancy (%)</td>
                {dates.map((d, i) => {
                  ...
                  return (
                    <td key={i} className="text-center px-3 py-3">
                      <span className={`font-bold text-base ${color}`}>{o.occupancyPct}%</span>
                    </td>
                  );
                })}
              </tr>
              {/* Total Available */}
              <tr className="border-b border-cream-dark">
                <td className="px-4 py-3 text-sm font-medium text-navy">Total Available</td>
                {dates.map((d, i) => {
                  const o = getOccupancyForDate(d);
                  return (
                    <td key={i} className="text-center px-3 py-3 text-navy font-semibold">
                      {o.available}
                    </td>
                  );
                })}
              ...
              {/* Total Booked */}
              <tr className="border-b border-cream-dark">
                <td className="px-4 py-3 text-sm font-medium text-navy">Total Booked</td>
                {dates.map((d, i) => {
                  const o = getOccupancyForDate(d);
                  return (
                    <td key={i} className="text-center px-3 py-3 text-navy font-semibold">
              ...
              {/* Total Blocked */}
              <tr>
                <td className="px-4 py-3 text-sm font-medium text-navy">Blocked</td>
                {dates.map((d, i) => {
                  const o = getOccupancyForDate(d);
                  return (
                    <td key={i} className="text-center px-3 py-3 text-blue-600 font-semibold">
                      {o.blocked}
                    </td>
                  );
                })}
            ...
            <table className="w-full min-w-[900px]">
              ...
              <tbody>
                {cat.ratePlans.map((plan) => (
                  <tr key={plan.code} className="border-b border-cream-dark last:border-b-0 hover:bg-cream/30 transition-colors">
                    <td className="px-4 py-3 text-sm font-bold text-navy">{plan.label}</td>
                    {dates.map((d, i) => {
      ...
      <div className="flex flex-wrap gap-4 text-xs mt-6 text-muted">
        ...
        <span className="text-gold-dark font-medium">💡 Click any price to edit it inline</span>
      </div>
    </div>
  );
}
