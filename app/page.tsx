...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      // DEBUG LOGS
      console.log("═══════════════════════════════════");
      console.log("[Dashboard] Total bookings loaded:", data.length);
      console.log("[Dashboard] Today's date:", todayISO());
      console.log("[Dashboard] Sample booking:", data[0]);
      console.log("[Dashboard] All statuses:", data.map((b) => b.status));
      console.log("[Dashboard] All checkIns:", data.map((b) => b.checkIn));
      console.log("[Dashboard] All bookingMadeOn:", data.map((b) => b.bookingMadeOn));
      console.log("[Dashboard] is_no_show values:", data.map((b) => (b as any).is_no_show));
      console.log("═══════════════════════════════════");
    } catch (err) {
  ...
  // Auto-refresh on focus + every 30s
  useEffect(() => {
  ...
  const stats = useMemo(() => {
    ...
    const newBookings = bookings.filter((b) => {
      const made = (b.bookingMadeOn || "").slice(0, 10);
      return made === today;
    });

    const inHouse = bookings.filter((b) => {
      if (b.status === "CHECKED-IN") return true;
      if (b.status === "CANCELLED" || b.status === "BLOCKED" || b.status === "ON-HOLD") return false;
      if (!b.checkIn || !b.checkOut) return false;
      return b.checkIn <= today && b.checkOut > today;
    });

    const arrivals = bookings.filter((b) => {
      return (b.checkIn || "").slice(0, 10) === today && b.status !== "CANCELLED";
    });

    const departures = bookings.filter((b) => {
      return (b.checkOut || "").slice(0, 10) === today || b.status === "PENDING DEPARTURE";
    });

    const cancellations = bookings.filter((b) => {
      return b.status === "CANCELLED" && !(b as any).is_no_show;
    });

    const onHold = bookings.filter((b) => b.status === "ON-HOLD");
  ...
  const filteredBookings = useMemo(() => {
    ...
    if (activeFilter) {
      ...
      if (key === "new-bookings") {
        if (value === "Today") {
          result = result.filter((b) => (b.bookingMadeOn || "").slice(0, 10) === today);
          ...
          result = result.filter((b) => {
            const made = (b.bookingMadeOn || "").slice(0, 10);
            return made >= weekStr && made <= today;
          });
          ...
          result = result.filter((b) => {
            const made = (b.bookingMadeOn || "").slice(0, 10);
            return made >= monthStr && made <= today;
          });
        }
        ...
        if (value === "Due Out Today") {
          result = result.filter((b) => (b.checkOut || "").slice(0, 10) === today);
        } else if (value === "Checked In") {
          result = result.filter((b) => b.status === "CHECKED-IN");
        }
      } else if (key === "arrivals") {
        result = result.filter((b) => (b.checkIn || "").slice(0, 10) === today);
        ...
        if (value === "Pending Departure") {
          result = result.filter(
            (b) => b.status === "PENDING DEPARTURE" || ((b.checkOut || "").slice(0, 10) === today && b.status === "CHECKED-IN")
          );
          ...
          result = result.filter(
            (b) =>
              (b.checkOut || "").slice(0, 10) === today ||
              b.status === "PENDING DEPARTURE" ||
              (b.status === "CHECKED-OUT" && (b.checkOut || "").slice(0, 10) === today)
          );
        }
      } else if (key === "cancellations") {
        result = result.filter((b) => b.status === "CANCELLED" && !(b as any).is_no_show);
              ...
              <>
                ...
                <div className="absolute top-full left-0 mt-2 z-50 bg-white border border-navy/10 rounded-lg shadow-xl p-3">
                  ...
                  <div className="flex gap-2 mt-2">
                    <button
                      onClick={() => { setSelectedDate(todayISO()); setDatePickerOpen(false); }}
                      className="text-xs text-navy px-3 py-1.5 border border-cream-dark rounded hover:bg-cream"
                    >
                      Today
                    </button>
                    <button
                      onClick={() => { setSelectedDate(yesterdayISO()); setDatePickerOpen(false); }}
                      className="text-xs text-navy px-3 py-1.5 border border-cream-dark rounded hover:bg-cream"
                    >
                      Yesterday
                    </button>
                ...
                <div key={stat.key} className="relative">
                  <button
                    ...
                    className={`w-full bg-white rounded-lg p-4 flex flex-col justify-between h-28 border-t-4 ${stat.color} text-left transition-all ${
                      isActive ? "shadow-lg ring-2 ring-gold -translate-y-1" : "shadow-sm hover:shadow-md hover:-translate-y-0.5"
                    }`}
                  >
                    <span className="font-serif text-3xl font-semibold text-navy leading-none">{stat.value}</span>
                        ...
                        {STAT_OPTIONS[stat.key].map((option) => (
                          <button
                            ...
                            className={`w-full text-left px-4 py-2.5 text-sm hover:bg-cream transition-colors flex items-center justify-between ${
                              isActive && activeFilter?.value === option ? "text-gold-dark font-semibold bg-cream/60" : "text-navy/80"
                            }`}
                          >
                            <span>{option}</span>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
            ...
            <div className="flex gap-3 items-center text-sm flex-wrap">
              <div className="flex items-center gap-2">
                ...
                <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="border border-cream-dark rounded-lg p-2 outline-none text-navy/80 bg-white text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <select value={pageSize} onChange={(e) => setPageSize(Number(e.target.value))} className="border border-cream-dark rounded-lg p-2 outline-none text-navy/80 bg-white text-sm">
                  <option value={25}>25</option>
                  <option value={50}>50</option>
                  <option value={100}>100</option>
                </select>
              </div>
              <button onClick={handleDownloadReport} className="border border-navy text-navy px-3 py-2 rounded-lg text-sm font-medium hover:bg-navy hover:text-cream transition">
                Download report
              </button>
            </div>
          </div>

          <p className="text-xs text-muted mb-3 text-right">{filteredBookings.length} results found</p>
            ...
            <div className="bg-white border border-cream-dark rounded-xl shadow-sm divide-y divide-cream-dark">
              {filteredBookings.length === 0 && (
                <div className="p-12 text-center text-muted">
                  ...
                  <p className="font-medium text-navy mb-1">{activeFilter ? "No bookings match this filter" : "No bookings yet"}</p>
                  {activeFilter && (
                    <button onClick={clearAllFilters} className="text-xs text-gold-dark underline mt-2">Clear all filters</button>
                  )}
                </div>
              )}
              {filteredBookings.slice(0, pageSize).map((b) => (
                <div key={b.id} className="p-4 grid grid-cols-1 md:grid-cols-12 gap-3 items-center text-sm hover:bg-cream/40 transition-colors">
                  ...
                  <div className="md:col-span-2 md:text-right">
                    <p className={`text-xs font-bold ${b.status === "PENDING DEPARTURE" ? "text-red-500" : b.status === "ON-HOLD" ? "text-purple-600" : "text-navy/70"}`}>
                      {b.status}
                    </p>
          ...
          {!loading && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
              ...
              <div className="bg-white border border-cream-dark rounded-xl shadow-sm p-5">
                ...
                <p className="font-serif text-3xl font-semibold text-rose-500">₹{totalOutstanding.toLocaleString("en-IN")}</p>
              ...
              <div className="bg-white border border-cream-dark rounded-xl shadow-sm p-5">
                ...
                <p className="font-serif text-3xl font-semibold text-emerald-600">₹{totalCollected.toLocaleString("en-IN")}</p>
